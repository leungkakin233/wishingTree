(function($, window) {
	...
	proto.initEvent = function() {
		...
		this.element.addEventListener('longtap', function() {
			mui('#saveAndShare').popover('show'); //显示
			mui('#saveAndShare').popover('hide'); //隐藏
			mui('#saveAndShare').popover('toggle'); //自动识别处理显示隐藏状态
		})
		...
		this.element.addEventListener('slide', function(e) {
			...
			document.getElementById("saveimg").addEventListener("tap",function(){
				mui('#saveAndShare').popover('hide'); //隐藏
//				var img_group=document.getElementsByClassName('preview');
				var target_group=whichgroup.getElementsByTagName('img');
//				var target_group=[];
//				for(var i=0;i<img_group.length;i++){
//					if(img_group[i].getAttribute('data-preview-group')==whichgroup){
//						target_group.push(img_group[i]);
//					}
//				}
			window.JsInterface.saveFile(target_group[self.lastIndex].src);
			mui.toast('已保存至手机相册');
		})
	};
