...
(function() {
	...
	feedback.newPlaceholder = function() {
		...
		up.addEventListener('tap', function(event) {
			plus.gallery.pick(function(event) {
				...
				filter: "image",
				multiple: true,
				maximum: 4 - feedback.files.length,
				system: false
			}); //至多上传4张图片
		}, false);
	...
	feedback.submitBtn.addEventListener('tap', function(event) {
		if (feedback.question.value == '' ||
			(feedback.contact.value != '' &&
				feedback.contact.value.search(
					/^(\w+((-\w+)|(\.\w+))*\@[A-Za-z0-9]+((\.|-)[A-Za-z0-9]+)*\.[A-Za-z0-9]+)|([1-9]\d{4,9})$/) != 0)) {
